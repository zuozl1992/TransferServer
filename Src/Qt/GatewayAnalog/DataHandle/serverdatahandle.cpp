...
void ServerDataHandle::handleControlCmdData(const QJsonObject &data)
{
    ...
    if(cmdType == "setNode1Light")
    {
        if((!data.value("i").isDouble()) ||
            ...
            return;
        DataCache::getObject()->setNode1Light(data.value("i").toInt(),
                                            data.value("l").toInt());
    }
    else if(cmdType == "setNode1RGBLED")
    {
        if((!data.value("g").isDouble()) ||
            ...
            return;
        DataCache::getObject()->setNode1RGBLED(data.value("g").toInt(),
                                             data.value("r").toInt(),
                                             data.value("b").toInt());
    }
    else if(cmdType == "setNode5AlarmLight")
    {
        if(!data.value("sw").isBool())
            return;
        DataCache::getObject()->setNode5AlarmLight(data.value("sw").toBool());
    }
    else if(cmdType == "setNode5AlarmBuzzer")
    {
        if(!data.value("sw").isBool())
            return;
        DataCache::getObject()->setNode5AlarmBuzzer(data.value("sw").toBool());
    }
    else if(cmdType == "setNode6SteeringGear")
    {
        if(!data.value("sw").isBool())
            return;
        DataCache::getObject()->setNode6SteeringGear(data.value("sw").toBool());
    }
    else if(cmdType == "setNode7Fun")
    {
        if(!data.value("sw").isBool())
            return;
        DataCache::getObject()->setNode7Fun(data.value("sw").toBool());
    }
    else if(cmdType == "setNode7Motor")
    {
        if((!data.value("dir").isDouble()) ||
            ...
            return;
        DataCache::getObject()->setNode7Motor(data.value("dir").toInt(),
    ...
    else if(cmdType == "setNode7Rec")
    {
        if((!data.value("id").isDouble()) ||
            ...
            return;
        DataCache::getObject()->setNode7Rec(data.value("id").toInt(),
    ...
    else if(cmdType == "setNode8Lock")
    {
        if(!data.value("sw").isBool())
            return;
        DataCache::getObject()->setNode8Lock(data.value("sw").toBool());
    }
}
